...
public sealed class McpKnowledgeIntegrationTests
{
    ...
    [Fact]
    public async Task Phase_three_registers_erp_mcp_adapter_with_tools_disabled_by_default()
    {
        var configuration = new ConfigurationBuilder().AddInMemoryCollection(new Dictionary<string, string?>
        {
            ["Mcp:KnowledgeBase:ServerName"] = "supabase-mcp-server_kb",
            ["Mcp:KnowledgeBase:Transport"] = "Stdio",
            ["Mcp:KnowledgeBase:Command"] = "npx",
            ["Ollama:Endpoint"] = "http://localhost:11434",
            ["Ollama:ChatModel"] = "qwen3:8b",
            ["Ollama:EmbeddingModel"] = "nomic-embed-text",
            ["Ollama:EmbeddingDimensions"] = "768"
        }).Build();
        await using var provider = new ServiceCollection().AddLogging().AddAiGateway(configuration).BuildServiceProvider();
        Assert.NotNull(provider.GetService<IErpMcpClient>());
        Assert.Empty(provider.GetRequiredService<IToolCatalog>().Enabled);
    }
