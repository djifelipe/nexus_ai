...
public sealed class CoreComponentTests
{
    ...
    {
        var router=new RuleBasedIntentRouter(new FakeCatalog(),Options.Create(new AiGatewayOptions()));var watch=Stopwatch.StartNew();
    ...
    {
        var router=new RuleBasedIntentRouter(new FakeCatalog(),Options.Create(new AiGatewayOptions()));var result=await router.RouteAsync(new("qual a previsão do tempo?",FiscalUser),default);Assert.Equal(IntentType.Unknown,result.Type);Assert.Null(result.Module);
    }
    ...
    public async Task Router_requests_clarification_for_equal_cross_module_matches_without_context()
    {
        ...
        var result=await new RuleBasedIntentRouter(new FakeCatalog(),Options.Create(new AiGatewayOptions())).RouteAsync(new("cancelamento",user),default);Assert.True(result.RequiresClarification);Assert.Contains("Fiscal",result.ClarificationQuestion);
    }
